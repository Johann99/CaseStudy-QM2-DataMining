...
# Train GBM model
set.seed(123)
gbm_model <- gbm(formula = fraud ~ ., data = df_train, interaction.depth = 8,
                 n.trees = 800, distribution = "bernoulli")
# not significant increase overall after 200, but specifically the alpha error is improving this is why we used n.trees=800
...
# Compute costs or standard prediction
cm2_gbm <- table(df_test$fraud, pred_test_gbm>0.53, dnn = c("Reality", "Prediction"))
cm2_gbm
...
# Compute costs or standard prediction
cm2_XGBoost <- table(df_test$fraud, pred_test_xgb>0.53, dnn = c("Reality", "Prediction"))
cm2_XGBoost
